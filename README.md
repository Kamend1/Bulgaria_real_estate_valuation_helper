...
Отделен, изцяло допълнителен панел на страницата за сравними — векторно търсене на семантично близки обяви от целия корпус (177K+ обяви) + LLM-генерирана чернова на обосновка. Работи зад абстракция, поддържаща три frontier доставчика (OpenAI/Anthropic/Google), с явна "чернова — изисква преглед" рамка навсякъде.
...
# AI-асистираната оценка (вижте секцията по-горе) -- без ключ приложението
# работи нормално, панелът просто показва "добавете ключ" вместо да генерира.
...
│   │   └── llm/                             # Phase 7 — RAG: providers, embeddings,
│   │                                        #   retriever, tools, valuation_chain
...
- AI-асистираната оценка използва само OpenAI за embeddings (смяна на provider означава re-embed на целия корпус); локален Qwen/Llama provider (`langchain-ollama`) все още не е добавен
- Снимки от оглед, сканирани правни документи и сертификат на оценителя все още не могат да се прикачват към доклада (изисква upload/attachment инфраструктура)
...
- Локален LLM provider (Ollama + Qwen/Llama) за AI-асистираната оценка
- Upload/attachment инфраструктура за снимки и сканирани документи в доклада
